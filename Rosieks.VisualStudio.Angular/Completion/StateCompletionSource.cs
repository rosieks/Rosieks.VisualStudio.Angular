namespace Rosieks.VisualStudio.Angular.Completion
{
    ...
    internal class StateCompletionSource : ICompletionSource
    {
        ...
        public StateCompletionSource(ITextBuffer textBuffer)
        {
    ...
    public static class UIRouterStateCompletionUtils
    {
        static readonly Regex regex = new Regex(@"(?<=\bstate\.go\(\s*(['""]))[a-z0-9_./+=-]*(?=\s*\1\s*\)?)?", RegexOptions.IgnoreCase);
